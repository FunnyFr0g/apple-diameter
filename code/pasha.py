...
def get_pos_desc(im1, im2, features_1, features_2, width=5):
    ...
    for y in range(features_1.shape[0]):
        for x in range(features_2.shape[1]):
            if features_1[y][x] != 0 and 0 <= y - width and y + width < features_1.shape[
                0] and 0 <= x - width and x + width < features_1.shape[1]:
                mean = 0.
                std = 0.
    ...
    for y in range(features_2.shape[0]):
        for x in range(features_2.shape[1]):
            if features_2[y][x] != 0 and 0 <= y - width and y + width < features_2.shape[
                0] and 0 <= x - width and x + width < features_2.shape[1]:
                mean = 0.
                std = 0.
...
def RANSACK(image1: np.ndarray, image2: np.ndarray, count_iterations: int, coridor: int, width=3) -> np.ndarray:
    ...
    base_features = cv2.dilate(cv2.cornerHarris(image1,2, 3, 0.04), None)
    dother_features = cv2.dilate(cv2.cornerHarris(image2,2, 3, 0.04), None)
    ...
    while i < count_iterations:
        ...
        while random_points[0] == random_points[1] or random_points[0] == random_points[2] or random_points[1] == \
                random_points[2]:
            random_points = np.random.randint(0, len(find), size=3)
    ...
    mixed_image = np.zeros(shape=(image1.shape[0] + image2.shape[0], image1.shape[1] + image2.shape[1] + 1),
                           dtype=np.uint8)
